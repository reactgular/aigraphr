...
export class WorkspacesService {
    ...
    private async load(): Promise<WorkspacesSchema> {
        try {
            ...
            console.error(
                `Error reading or parsing ${this.workspacePath}:`,
                error
            );
            throw error;
        }
    ...
    private async prompts(
        workspace: WorkspacesSchema
    ): Promise<WorkspacesSchema | null> {
        const values = await prompts.group(
            {
                ...
                langType: () =>
                    prompts.select<
                        Option<WorkspaceLanguage>[],
                        WorkspaceLanguage
                    >({
                        message: 'Pick a project type.',
                        options: [
                            {
                                value: WorkspaceLanguage.TYPESCRIPT,
                                label: 'TypeScript'
                            },
                            {value: WorkspaceLanguage.PYTHON, label: 'Python'}
                        ],
                        initialValue:
                            workspace?.langType ?? WorkspaceLanguage.TYPESCRIPT
                    }),
                plugins: () =>
                    prompts.multiselect({
